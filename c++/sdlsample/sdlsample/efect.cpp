...
void loadSounds()
{
    ...
    //読み込み
    se[0]=Mix_LoadWAV("se0.aif");
    se[1]=Mix_LoadWAV("se1.aif");
    //BGM
    BGM[0]=Mix_LoadMUS("BGM0.ogg");
    BGM[1]=Mix_LoadMUS("BGM1.ogg");
    int lop;
...
void efect_main()
{
    ...
    for(lop=0;		lop<efcMax;		lop++){
        if(eft.flg[lop]>=1)
        {
            ...
            Uint8 R=rand()%256;
            Uint8 G=rand()%256;
            SDL_SetRenderDrawColor(bkRen, R, G, 0, 255);
            SDL_RenderFillRect(bkRen, &efe);
            //表示カウンタ
...
void efect_main2()
{
    ...
    for(lop=0;		lop<efcMax;		lop++){
        if(eft2.flg[lop]>=1)
        {
            ...
            Uint8 B=rand()%256;
            SDL_SetRenderDrawColor(bkRen, 0, G, B, 255);
            SDL_RenderFillRect(bkRen, &efe);
            //表示カウンタ
